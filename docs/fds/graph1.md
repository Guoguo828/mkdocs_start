...
3. **多重链表（十字链表）（有向图）**  
   - 一个行的链表，一个列的链表。（共用一个数组，一个元素两个指针。）  
   - **行链表**：存储从这个点发出的点。  
   - **列链表**：存储进入这个点的点。  
   - 每个节点包含：  
     - 两个图中的节点，分别代表出发点和到达点，其实就是一条边嘛。  
     - 两个指针：  
       - 左指针：指向第一个点的下一个进入点所在的节点（横着的）。  
       - 右指针：指向第二个点的下一个被进入点所在的节点（竖着的）。
   - **优点**：可以快速找到指向某个点的节点，自己找自己。
   - **无向图也可以用（邻接多重表）**：把所有包含这个点的节点都放入这个点的链表（链表的节点还是两个点和两个指针），注意指针要是这个点管的指针（左 or 右）。
