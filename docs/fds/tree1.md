...
通过上面的原理，我们就可以知道了，只有一个操作是push的时候，才相当于找到了一个新节点。当这个push操作的前面一个操作是入栈，那么这个操作对应的点就左儿子，当这个push操作的前面一个操作是出栈那么这个操作对应的点就是右儿子。因此我们可以通过这个来建立二叉树。
