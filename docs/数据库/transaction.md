...
# 三张PPT完整解读：可恢复调度、级联回滚、无级联调度
这是数据库事务并发调度（Schedule）的**故障恢复安全分级**，解决并发事务崩溃时的数据一致性问题，三者是层层加强的约束关系：
## 1. 第一张：Recoverable Schedules 可恢复调度
### 定义规则
如果事务 $T_j$ 读取了事务 $T_i$ 之前修改写入的数据，**必须保证 $T_i$ 先提交 commit，$T_j$ 才能提交 commit**。
$$write_i(A) \rightarrow read_j(A) \implies commit_i \rightarrow commit_j$$
### 反例（图中 $T_8,T_9$）
调度流程：
$T_8:read(A) \rightarrow write(A)$
$T_9:read(A) \rightarrow commit$
$T_8:read(B)$
问题：$T_9$ 读完脏数据立刻提交了，但修改数据的 $T_8$ 还没提交。
- 若后续 $T_8$ 崩溃回滚 abort，$T_9$ 已经拿着脏数据提交，数据库出现不一致；
- 这种调度**不可恢复**，数据库必须禁止。
### 核心作用
杜绝「脏读之后先提交」，保证崩溃后数据库能回滚到一致状态，是并发调度最基础的安全底线。

## 2. 第二张：Cascading Rollbacks 级联回滚
### 场景前提
调度满足**可恢复**，但依然有缺陷：允许读取未提交事务的脏数据。
### 定义
一个事务崩溃回滚，会连锁导致所有读过它脏数据的事务全部跟着回滚。
### 例子（$T_{10},T_{11},T_{12}$）
1. $T_{10}$ 修改A；
2. $T_{11}$ 读取 $T_{10}$ 未提交的A、再修改A；
3. $T_{12}$ 读取 $T_{11}$ 未提交的A；
4. $T_{10}$ 中途 abort 崩溃。

后果：
$T_{11}$ 读了 $T_{10}$ 的脏数据、$T_{12}$ 读了 $T_{11}$ 的脏数据，因此 $T_{11},T_{12}$ 必须全部跟着回滚。
### 缺点
大量已执行的业务操作全部作废，系统性能损耗极大。

## 3. 第三张：Cascadeless Schedules 无级联调度（也叫避免级联回滚调度）
### 定义规则（约束比可恢复更强）
如果 $T_j$ 要读取 $T_i$ 修改的数据，**必须等 $T_i$ 完整提交 commit 之后，$T_j$ 才能执行 read 读操作**。
$$write_i(A) \rightarrow read_j(A) \implies commit_i \rightarrow read_j(A)$$
### 关键结论
1. **所有无级联调度，一定是可恢复调度**（约束更强，天然满足可恢复的规则）；
2. 彻底消除级联回滚：事务永远只会读取其他事务**已经提交**的数据；
   就算某事务崩溃，其他事务不会读到它的脏数据，不需要连锁回滚；
3. 工程上数据库普遍优先采用无级联调度，减少崩溃后的回滚开销。

# 三者约束强弱关系
$$\text{Cascadeless（无级联）} \subset \text{Recoverable（可恢复）}$$
1. 可恢复调度：只限制「脏读之后不能先提交」，允许读未提交数据，会产生级联回滚；
2. 无级联调度：直接禁止「读取未提交事务的数据」，从根源杜绝级联回滚，安全性更高。

# 一句话通俗总结
1. 可恢复调度：别人改的数据没提交，你可以读，但你不能比他先交；
2. 级联回滚：你读了别人没提交的数据，一旦对方崩了，你也得全部撤销；
3. 无级联调度：必须等别人改完、提交成功后，你才能读，不会出现连锁撤销。


# 幻灯片《Implementation of Isolation 隔离性的实现》完整解读
这一页承接前面**可恢复/无级联调度**，讲数据库并发控制的整体设计目标、取舍与分类。

## 1. 第一条：串行执行方案的缺点
> A policy in which only one transaction can execute at a time generates serial schedules, but provides a poor degree of concurrency.
- 方案：同一时刻只允许一个事务运行（完全串行执行）
- 优点：天然生成**串行调度 serial schedule**，绝对不会有并发冲突、一致性100%保证；
- 致命缺点：并发度极低，同一时间只能跑一个事务，数据库吞吐量极差，生产环境不会这么用。
- 引申：数据库追求**并发 + 一致性兼顾**，所以要支持交错执行，但必须对交错的调度加约束。

## 2. 第二条：合格调度的三层硬性要求（数据库一致性底线）
...
一份安全、能保证数据一致的并发调度，必须同时满足两类约束，优先级从强制到优选：
1. **可串行化（Serializable，隔离性核心）**
    二选一即可：冲突可串行化 conflict-serializable / 视图可串行化 view-serializable；
    作用：保证并发执行等价于某一个串行顺序执行，不会出现并发逻辑错误（丢失更新、不可重复读等）。
2. **可恢复（Recoverable，故障安全强制要求）**
    就是前一页讲的：读了别人未提交修改的事务，不能比修改方先提交；崩溃后数据库能正确回滚，不产生永久脏数据。
3. **优选：无级联（Cascadeless，工程优化）**
    不强求，但推荐；直接禁止读取未提交数据，彻底杜绝级联回滚，崩溃时回滚开销大幅降低。

一句话概括：
调度先做到「可串行化」保证逻辑正确，再做到「可恢复」保证崩溃安全，最好再做到「无级联」减少故障代价。

## 3. 第三条：并发控制算法的核心权衡（trade-off）
> Concurrency-control schemes tradeoff between the amount of concurrency they allow and the amount of overhead that they incur.
所有并发控制方案（两阶段锁2PL、时间戳、MVCC等）都存在**此消彼长的取舍**：
- 一端：允许多大的并发度（事务能多大程度交错执行，越高吞吐量越好）；
- 另一端：系统额外开销（锁竞争、版本存储、日志、校验计算等，开销越低性能越好）。

举例直观理解：
- 严格锁机制：限制并发、开销低；
- MVCC多版本：支持超高并发，但要存储多版本数据、维护清理版本，开销更高。

## 4. 第四条：两类可串行化对应的并发控制算法区分
...
区分两种可串行化的实现：
1. **冲突可串行化 Conflict-serializable**
    大多数经典锁算法（如2PL）只能生成这类调度；约束更强，实现简单、检测方便，但会拒绝一部分合法并发调度，并发上限更低。
2. **视图可串行化 View-serializable**
    范围更大：所有冲突可串行化都是视图可串行化，但存在**视图可串行、但不冲突可串行**的合法调度；
    少数算法（如基于验证的乐观并发控制）能支持这一类，允许更多并发交错，但检测逻辑复杂、系统开销更大。

# 整体串联总结
1. 纯串行执行最安全但并发太差，数据库必须引入并发调度；
2. 合法调度双重强制标准：**可串行化（逻辑正确） + 可恢复（崩溃安全）**，工程尽量做到无级联；
3. 所有并发控制算法都要平衡「并发吞吐量」和「系统运行开销」；
4. 不同算法支持的可串行化等级不同：锁机制大多只支持冲突可串行，部分乐观算法能支持范围更广的视图可串行。
